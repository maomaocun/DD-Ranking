...
func_names = ['rotate', 'saturation', 'brightness', 'contrast', 'scale', 'crop', 'cutout', 'flip']
...
for ipc in [1, 10, 50]:
    ...
    save_path_dsa = f"./results/{dataset}/{model_name}/IPC{ipc}/dc_dsa_scores.csv"
    convd3_dsa_obj = DSA_Augmentation_Metrics(dataset=dataset, real_data_path=data_dir, ipc=ipc, model_name=model_name, device=device, 
                                              func_names=func_names, params=dsa_params, aug_mode="S", save_path=save_path_dsa)
